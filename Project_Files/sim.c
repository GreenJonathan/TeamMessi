...
static void run_sjf(SimProc *sp, int n, SimParams p, SimStats *out, int opt) {
    const int half = p.t_cs / 2;
    g_sp = sp;
    g_hsize = 0;
    rq_init();
    ...
    int sim_end     = 0;
    long cpu_busy   = 0;
    const char *nm = algo_name(ALGO_SJF, opt);
    ...
    while (g_hsize > 0) {
        ...
        case EV_CPU_DONE: {
            int just = proc->burst_idx;
            cpu_busy += (long)(t - proc->cpu_start_time);
            if (!opt && p.alpha > 0.0) {
                int actual = proc->cpu_bursts[just];
                proc->tau = p.alpha * actual + (1.0 - p.alpha) * proc->tau;
            }
            int bursts_left = proc->num_bursts - proc->burst_idx - 1;
            proc->total_ta += (long)(t + half - proc->burst_rq_enter);
            proc->bursts_done++;
...
static void run_srt(SimProc *sp, int n, SimParams p, SimStats *out, int opt) {
    const int half = p.t_cs / 2;
    g_sp = sp;
    g_hsize = 0;
    rq_init();
    ...
    int sim_end     = 0;
    long cpu_busy   = 0;
    const char *nm = algo_name(ALGO_SRT, opt);
    ...
    while (g_hsize > 0) {
        ...
        case EV_ARRIVE: {
            ...
            int run = cpu_running;
            if (run >= 0) {
                int rem = sp[run].remaining - (t - sp[run].cpu_start_time);
                int nk  = key_sjf(sp, pi, opt);
                if (nk < rem || (nk == rem && strcmp(sp[pi].id, sp[run].id) < 0)) {
                    printf("time %dms: Process %s arrived; preempting %s "
                           "(remaining time %dms) [Q: -]\n",
                           t, sp[pi].id, sp[run].id, rem);
                    SimProc *v = &sp[run];
                    cpu_busy += (long)(t - v->cpu_start_time);
                    v->remaining -= (t - v->cpu_start_time);
                    v->run_gen++;
                    v->num_preempt++;
                    v->ready_at = t + half;
                    rq_insert_sorted(run, key_sjf, opt);
                    cpu_free_at = t + half;
                    cpu_running = -1;
                    cpu_running = pi;
                    ev_push((Event){ t + p.t_cs, EV_CPU_START, pi, 0 });
                    break;
                }
            }
            rq_insert_sorted(pi, key_sjf, opt);
        ...
        case EV_CPU_START:
            ...
            proc->total_wait += (long)(t - half - proc->ready_at);
            proc->num_cs++;
            if (proc->remaining == proc->cpu_bursts[proc->burst_idx])
                printf("time %dms: Process %s started using the CPU for %dms burst ",
                       t, proc->id, proc->remaining);
            else
                printf("time %dms: Process %s started using the CPU for "
                       ...
                       t, proc->id, proc->remaining,
                       proc->cpu_bursts[proc->burst_idx]);
            print_rq();
            printf("\n");
...
static void run_rr(SimProc *sp, int n, SimParams p, SimStats *out) {
    ...
    while (g_hsize > 0) {
        ...
        case EV_CPU_START:
            ...
            {
                int g = proc->run_gen;
                ev_push((Event){ t + proc->remaining, EV_CPU_DONE, pi, g });
                if (proc->remaining > Q)
                    ev_push((Event){ t + Q, EV_SLICE_EXP, pi, g });
            }
            break;

        case EV_SLICE_EXP:
            if (e.gen != proc->run_gen || cpu_running != pi) break;
            if (rq_empty()) {
                printf("time %dms: Time slice expired; no preemption because "
                       "ready queue is empty [Q: -]\n", t);
                ev_push((Event){ t + Q, EV_SLICE_EXP, pi, e.gen });
                break;
            }
            {
                int rem = proc->remaining - (t - proc->cpu_start_time);
                printf("time %dms: Time slice expired; preempting process %s "
                       "with %dms remaining ", t, proc->id, rem);
        ...
        case EV_CPU_DONE:
            ...
            cpu_busy += (long)(t - proc->cpu_start_time);
            {
                ...
                proc->total_ta += (long)(t + half - proc->burst_rq_enter);
                proc->bursts_done++;
                if (proc->cpu_bursts[proc->burst_idx] <= Q)
...
void run_simulation(const Process *procs, int n, AlgoType algo,
    ...
    SimProc *sp = calloc((size_t)n, sizeof(SimProc));
    ...
    case ALGO_FCFS:
        ...
        break;
    case ALGO_SJF:
        run_sjf(sp, n, params, out, opt);
        break;
    case ALGO_SRT:
        run_srt(sp, n, params, out, opt);
        break;
    case ALGO_RR:
